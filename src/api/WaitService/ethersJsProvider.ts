...
const providers: {
  [key: string]: (
    provider: string,
    networkOptions?: Network | undefined
...
providers.moonbeam = (provider: string, networkOptions: Network | undefined) =>
  new ethers.providers.StaticJsonRpcProvider(provider, networkOptions);
providers.polygon = (provider: string, networkOptions: Network | undefined) =>
  new ethers.providers.StaticJsonRpcProvider(provider, networkOptions);
...
export const getEthersJsProvider = (
  ...
  const provider: string = getConfigs(environment).ethersJsConfigs[
    providerType as string
  ]?.providerOptions.provider as string;
  const networkOptions: Network | undefined =
    getConfigs(environment).ethersJsConfigs[providerType as string]
