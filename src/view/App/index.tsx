	...
	render: (message: string, onConfirm: () => void, onCancel: () => void) => {
		return (
			<StyledDialogBox>
				...
				<br/>
				<FlexRow>
					<StyledButton style={{margin: `0.5em`}} onClick={onConfirm}> Okay </StyledButton>
				</FlexRow>
			</StyledDialogBox>
		);
	}
...
const App = () => {
	...
	const [hasAcknowledgedTerra, setHasAcknowledgedTerra] = useRecoilState(HasAcknowledgedTerraReinstall);
	...
	useEffect(() => {
		if (!hasAcknowledgedTerra) {
			const message: any = <FlexColumn>
				...
				<br/>
				<div><BoldSpan>
					If you previously added it to your Keplr wallet through Satellite, please delete Terra from the
					extension and add it back with the <SVGImage
					height={`1em`}
					width={`1em`}
			...
			confirm(message, options as any).then(acknowledged => setHasAcknowledgedTerra(true))
		}
	}, [hasAcknowledgedTerra, setHasAcknowledgedTerra])
