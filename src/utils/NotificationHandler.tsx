...
const traceIdDiv = (traceId: string | undefined) => {
	if (!traceId)
		return null;
	return <div><br/>For support, reach out on Discord in the <BoldSpan>#satellite-open-support-ticket</BoldSpan> channel
		with this <Tooltip
...
const getMessageType = (error: any): JSX.Element => {
	...
	return MessageContent(getNotificationMessage(error.statusCode, error), [504,403,403.1,429].includes(error.statusCode) ? null : error.traceId);
