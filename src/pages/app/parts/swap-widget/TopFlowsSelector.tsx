...
const StyledHelperComponent = styled.div<IStyledDivProps>`
  ...
  background-color: ${(props) =>
    props.appear ? `none` : props.theme.headerBackgroundColor};
...
const TopFlowsPopup = styled.div<IStyledDivProps>`
  ...
  background-color: ${(props) =>
    props.appear ? `rgba(0, 0, 0, 0.05)` : `none`};
  transform: ${(props) =>
    props.appear
...
const FlowOption = (props: IFlowOptionProps) => {
  ...
  return (
    <Tooltip
      width={"60%"}
      anchorContent={
        <StyledFlowOption onClick={() => props.onClick(props)}>
...
const TopFlowsSelector = ({
  ...
  const setDestinationChain = useSetRecoilState(
  ...
  const topChainsMap = useMemo(() => {
    ...
    map.Osmosis = chainList.find(
      (chainInfo) => chainInfo.chainName === "Osmosis"
    ) as ChainInfo
    map.Ethereum = chainList.find(
      (chainInfo) => chainInfo.chainName === "Ethereum"
    ) as ChainInfo
    map.Axelar = chainList.find(
      (chainInfo) => chainInfo.chainName === "Axelar"
    ) as ChainInfo
    map.Fantom = chainList.find(
      (chainInfo) => chainInfo.chainName === "Fantom"
    ) as ChainInfo
  ...
  const onClick = (selection: IFlowOptionProps) => {
    ...
    setSourceAsset(
      map[selection.sourceChainName]?.assets?.find(
        (asset) => asset.common_key === selection.assetCommonKey
      ) as AssetInfo
    )
    setShowFlows(false)
    closeAllSearchWindows()
